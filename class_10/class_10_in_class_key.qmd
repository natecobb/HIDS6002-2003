...
WITH possible_matches AS (
  ...
  AS match_score,
  matches.*
  FROM pmc.all_patient_records target
   LEFT JOIN pmc.all_patient_records matches
   ON 1 = 1
...
eids <- 
  dbGetQuery(con, "SELECT enterpriseID from pmc.all_patient_records WHERE first = 'JAMES' and last = 'SMITH' LIMIT 100")
